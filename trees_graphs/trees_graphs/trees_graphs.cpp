...
//4.9 BST Sequence
void AppenDepth(binary_tree_node* node, int depth = 0) {
    if (node == nullptr)
        return;

    node->depth += depth;
    depth++;
    AppenDepth(node->left, depth);
    AppenDepth(node->right, depth);
}

vector<vector<int>>& GetDepth(binary_tree_node* node, vector<vector<int>>& depth_map ) {
    ...
    GetDepth(node->left,depth_map);
    depth_map[node->depth].push_back(node->val);
    GetDepth(node->right,depth_map);
    

    return depth_map;
}

void BSTSequence(binary_tree_node* node) {
    vector<vector<int>> depth_map(5, vector<int>(1, -1));
    AppenDepth(node);
    GetDepth(node, depth_map);

    typename vector<vector<int>>::iterator it;
    typename vector<int>::iterator it1;
    map<int, vector<int>> xx;

    vector<int> old_versions;
    int i = 0;
    //for (it = depth_map.begin(); it != depth_map.end(); it++)
    //{
    //    for (it1 = it->begin() + 1; it1 != it->end(); it1++)
    //    {
    //        //cout << *(it1) << " ";
    //        xx[i].push_back(*it1);
    //    }
    ...
    //        for (it2 = xx[i].begin(); it2 != xx[i].end(); it2++)
    //        {
    //            /*cout << *it2;*/
    //            if (!old_versions.empty()) {
    //                for (vector<int>::iterator it3 = old_versions.begin(); it3 != old_versions.end(); it3++)
    //                {
    //                    cout << *it2 << " " << *it3 << endl;;
    //                }
    //            }
    ...
    //            }
    //            
    //            old_versions.push_back(*it2);
    //        }
    //        cout << endl;
    //       

    //    }         while (next_permutation(xx[i].begin(), xx[i].end()));
...
int main()
{
   ...
   int arr[] = {1,2,3};
     root =  create_minimal_tree(arr,0,2);
     BSTSequence(root);
 /*    tree_traversal(root);*/
